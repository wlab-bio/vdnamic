...
def generate_ueis(uei_arr,sim_pos,sorted_unif_rand_res,cumul_rate,Nbcn,Ntrg,Nuei,sim_dims,phys_dims,t_terms,tot_rate,uei_ind):
    uei_ind_thiscall = 0
...
def sum_partition_function(sim_pos,Nbcn,Ntrg,sim_dims,phys_dims,t_terms):
    my_sum = 0.0
    for i in range(Nbcn):
        for j in range(Nbcn,Nbcn+Ntrg):
            sqdist = 0.0
...
def sim_gse():
    ...
    raw_image_csv = raw_image_csv[np.argsort(raw_image_csv[:,1]),:] # type 1 followed by type 2
    Npt1 = np.sum(raw_image_csv[:,1]==0)
    Npt2 = np.sum(raw_image_csv[:,1]==1)
    sim_pos = np.array(raw_image_csv[:,2:],dtype=np.float64)
    # rescale points to ~4 unit radius
    sim_pos *= 2/np.sqrt(np.sum(np.var(sim_pos,axis=0)))
...
def sim_reads(Nbcn,Ntrg,Nuei,Nuei_rca,sim_pos):
    ...
    for for_uei_i in range(Nuei + Nuei_rca):
        ...
        if filename.endswith('_sim_ueifile.csv'):
            ...
            for i in range(ueifile.shape[0]):
                for myread in range(ueifile[i,3]):
                    read_list.append([np.array(ueifile[i,np.arange(5,dtype=np.int64)!=3])])
            ...
            for i in range(read_list.shape[0]):
                for_param_index = int(read_list[i,3]) # uei classification
...
if __name__ == '__main__':
    ...
    sysOps.sh("awk -F, 'BEGIN{on_uei=0;}{for(i=1;i<=$4;i++){print on_uei \",\" $2 \",\" $3 \",\" 2 \",0\"; on_uei++;}}' p0.8//link_assoc.txt > p0p8_sim_ueifile.csv")
    Nuei_rca = 0
    posfilename = "posfile.csv"
    ...
    Npt2 = np.sum(raw_image_csv[:,1]==1)
    sim_pos = np.array(raw_image_csv[:,2:],dtype=np.float64)
    Nuei = int(np.sum(np.loadtxt('p0.8//link_assoc.txt',delimiter=',')[:,3]))
    Nuei_rca = 0
    sim_reads(Npt1,Npt2,Nuei,Nuei_rca,sim_pos)
